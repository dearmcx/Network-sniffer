...
int net_filter(struct xdp_md *ctx)
{
  ...
  if ((void *)eth + sizeof(*eth) <= data_end)
  {
    ...
    //show_data->pid = bpf_get_current_pid_tgid() >> 32;
    //show_data->tgid = bpf_get_current_pid_tgid(); 
    //bpf_get_current_comm(&show_data->comm, sizeof(show_data->comm));
    if(eth->h_proto == htons(ETH_P_IPV6)||eth->h_proto == htons(ETH_P_IP)||eth->h_proto == htons(ETH_P_ARP)){
    	...
    	bpf_probe_read_kernel(&(show_data->smac), mac_len, &eth->h_dest);
    	bpf_probe_read_kernel(&(show_data->dmac), mac_len, &eth->h_source);
    }
